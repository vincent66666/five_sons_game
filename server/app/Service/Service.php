...
class Service
{
    ...
    public function __get($key)
    {
        ...
        if ($suffix && ($suffix == 'Repo' || $suffix == 'Repository')) {
            ...
            return $this->getRepositoriesInstance($repoName, static::class, 12);
        }
        if (substr($key, -7) == 'Service') {
            return $this->getServiceInstance($key, static::class, 12);
        }
        if (substr($key, -5) == 'Model') {
            $key = strstr($key, 'Model', true);
            return $this->getModelInstance($key, static::class, 12);
        }
        if (substr($key, -3) == 'Mdl') {
